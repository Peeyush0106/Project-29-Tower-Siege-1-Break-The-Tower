...
function draw() {
    ...
    towerTable1.display("red");//Tower table 1 display
    towerTable2.display("red");//Tower table 2 display
    //Display cup placed on the left side tower table
    for (var l = 0; l < leftCups.length; l++) {
        var cup = leftCups[l];
        cup.display("orange");
    }
    ...
    if (canRestart(polygon.body.velocity.x < 2
    ...
    //When the polygon went out of the screen, we will give another chance to the player
...
//When to restart the game. The required conditions should be met
function canRestart(requiredConditions) {
